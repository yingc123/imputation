...
TEAM_ID_FOR_ROUND1_inv = {
    ...
    'Average':20,
    'BrokenNodes':1000,
    'ENCODE_DCC_Imputes':1010,
    'Hongyang_Li_and_Yuanfang_Guan':1020,
    'KKT-ENCODE-Impute-model_1':1030,
    'KKT-ENCODE-Impute-model_2':1031,
    'LiPingChun':1040,
    'MLeipzig':1050,
    'noml':1060
}
...
def read_scores_from_csv(csv_file, chroms):
    ...
    result = []
    with open('/home/ying/PycharmProjects/test/imputation_challenge/round1.csv', mode='r') as csv_file:
...
def calc_combined_ranks(rows, measures_to_use):
    ...
    team_ids = [TEAM_ID_FOR_ROUND1_inv[x['team']]for x in rows]
    ...
    for user_i, attr in enumerate(measures_to_use):
        ...
        attr_scores = numpy.array([x[attr] for x in rows])

        attr_scores = attr_scores.astype('float64')
...
def calc_global_ranks(rows, measures_to_use):
    ...
    for x in rows:
        ...
        sample_grpd_results[(x['cell_id'], x['assay_id'])][x['bootstraip_id']].append(x)
        all_users.add(TEAM_ID_FOR_ROUND1_inv[x['team']])
    ...
    for (cell, assay), bootstrapped_submissions in sample_grpd_results.items():
        ...
        file = open('/home/ying/Cluster/izkf/projects/ENCODEImputation/report/Round1Rank/rank.txt', 'a')
        #file.write('Team\tcell\tget_cell_name(cell)\tassay\tget_assay_name(assay)')

        #file.write('\t'.join(('Team', 'name', 'rank')))
        #file.write('\n')
        #file.write('|'.join(('----',)*3))
        for (team_id, submission_id), ranks in sorted(
                user_ranks.items(), key=lambda x: sorted(x[1])[1]):
            file.write('%d\t%s\t' % (team_id, get_team_name(team_id)))
            file.write('{}\t{}\t{}\t{}\t'.format(cell, get_cell_name(cell), assay, get_assay_name(assay)))
    ...
    #print()
    file = open('/home/ying/Cluster/izkf/projects/ENCODEImputation/report/Round1Rank/rank_all.txt', 'a')
    #file.write('#\tOverall\tResults\n')
    file.write('\t'.join(('Team name', 'rank', 'Lower bound',
                      'Mean', 'Upperbound')))
    file.write('\n')
    #print('|'.join(('----',)*6))
...
def main():
    ...
    rows = read_scores_from_csv(args.csv_file, args.chrom)
