...
emb_loc = "/home/ey712185/EncodeImputation/result/embedding_weights"
save_loc = '/home/ying/Cluster/home/EncodeImputation/result/vis'

def load_dict(type, lst):
    result_dict = {}
    for j in lst:
        result_dict[j] = []
    for i in chrom_list[-2:-1]:
        para = np.load(os.path.join(emb_loc, '{}_{}.npy'.format(type, i)), allow_pickle=True)
...
def plot_tsne(dic, data, type,lst):
    ...
    f = open(os.path.join(save_loc, '{}.txt'.format(type)), 'a')
    for i in Y:
        #print(i)
    ...
    for j in range(len(lst)):
        plt.text(Y[j, 0], Y[j, 1], y[j], fontdict={'weight': 'bold', 'size': 9})
    plt.savefig(os.path.join(save_loc, '{}_embedding.pdf'.format(type)), format='pdf')
